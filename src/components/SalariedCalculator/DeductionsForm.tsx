...
interface DeductionsFormProps {
  ...
  onNext: (values: SalariedFormData) => void;
  onBack: () => void;
}

const DEDUCTION_FIELDS = [
  { name: 'hra', label: 'House Rent Allowance (HRA)', placeholder: 'Enter HRA amount' },
  { name: 'lta', label: 'Leave Travel Allowance (LTA)', placeholder: 'Enter LTA amount' },
  { name: 'section80C', label: 'Section 80C Deductions', placeholder: 'Enter 80C deductions' },
  { name: 'other', label: 'Other Deductions', placeholder: 'Enter other deductions' }
];

const validationSchema = Yup.object({
  deductions: Yup.object({
    hra: Yup.number().min(0, 'Must be positive'),
    lta: Yup.number().min(0, 'Must be positive'),
    section80C: Yup.number().min(0, 'Must be positive'),
